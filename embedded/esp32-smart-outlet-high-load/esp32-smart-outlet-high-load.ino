...
#define RELAY_PIN 13      // GPIO13 for relay control (normally open - HIGH = off/open, LOW = on/closed)
#define STATUS_LED 4     // GPIO4 is the built-in flash LED on ESP32-CAM

// Note about pin choices for ESP32-CAM:
// - Using GPIO13 for relay (safe digital output)
// - Using built-in flash LED (GPIO4) for status indication
...
void setup() {
  ...
  // Start with relay off (circuit open) and LED off
  digitalWrite(RELAY_PIN, HIGH);  // HIGH = relay off (circuit open)
  ...
  // WiFi Manager setup with custom settings
  ...
  // Configure WiFiManager for better stability
  wifiManager.setConnectTimeout(30); // 30 seconds to connect
  wifiManager.setConfigPortalTimeout(180); // 3 minutes config portal timeout
  
  // Reduce WiFi power during AP mode
  WiFi.setTxPower(WIFI_POWER_19_5dBm);
  
  // Try to connect to WiFi with retry logic
  bool connected = false;
  for(int i = 0; i < 3 && !connected; i++) { // Try up to 3 times
    connected = wifiManager.autoConnect("ESP32-SmartOutlet");
    if(!connected) {
      delay(1000); // Wait a bit before retry
      digitalWrite(STATUS_LED, !digitalRead(STATUS_LED)); // Toggle LED to show retry
    }
  }
...
void updateRelayState(bool state) {
  ...
  digitalWrite(RELAY_PIN, state ? LOW : HIGH);  // Inverted logic: LOW = on/closed, HIGH = off/open
  ...
  // Keep LED on if relay is on, off if relay is off
...
void checkDeviceStatus() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    digitalWrite(STATUS_LED, LOW); // Turn off connection LED
